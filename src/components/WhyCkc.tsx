        ...
        <div className="mt-14 grid gap-4 sm:grid-cols-2">
          {differentiators.map((item, index) => (
            <Reveal key={item.title} delay={index * 0.06}>
              <article className="h-full rounded-3xl border border-white/10 bg-panel/70 p-7 transition duration-300 hover:border-sage/35">
                <p className="font-mono text-xs text-sage">0{index + 1}</p>
                <h3 className="mt-4 font-serif text-2xl text-stone">{item.title}</h3>
          ...
          <div className="grid gap-4 rounded-3xl border border-white/10 bg-ink-2/80 p-6 sm:grid-cols-3 sm:p-8">
            {[
              ...
              <div key={stat.k} className="border-white/8 py-2 sm:border-l sm:px-6 sm:first:border-l-0 sm:first:pl-0">
                <p className="font-mono text-[0.68rem] tracking-[0.18em] text-brass uppercase">
                  {stat.k}
                </p>
                <p className="mt-2 font-serif text-2xl text-stone">{stat.v}</p>
              </div>
            ))}
          </div>
        </Reveal>
